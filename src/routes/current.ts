...
router.get(
    ["/element/:element/:interval", "/element/:element"],
    protect("read"),
    ...validateUnits,
    param("interval")
        .default("live" as IntervalDescriptor)
        .isIn(IntervalDescriptors)
        .withMessage("Invalid or missing interval entered!"),
    param("element")
        .isIn(CurrentConditionsElements)
        .withMessage("Invalid weather element!"),
    asyncHandler(async (req, res, next) => {
        const result: Result<ValidationError> = validationResult(req);

        if (result.isEmpty()) {
            const records = await CurrentConditions.find({
                interval:
                    intervalOf(req.params.interval as IntervalDescriptor) || 0,
            })
                .sort({
                    time: "asc",
                })
            ...
            if (records.length === 0) {
                return next(
                    new APIError(
                        "No current weather conditions available. This error usually happens if no weather data hasn't been uploaded!",
                        503
                    )
                );
            }
            ...
            for (let i = 0; i < records.length; ++i) {
                records[i].changeUnits({
                    ...
                    solarRadiation: req.query
                        .solarRadiationUnit as SolarRadiationUnit,
                    pressure: req.query.pressureUnit as PressureUnit,
                });

                const result = Object.assign(records[i].toJSON()) as any;
                result.value =
                    result[req.params.element as CurrentConditionsElement];
            ...
            res.json({
                success: true,
                data:
                    (req.params.interval as IntervalDescriptor) === "live"
                        ? results[0]
                        : results,
            });
        } else {
    ...
    asyncHandler(async (req, res, next) => {
        ...
        if (result.isEmpty()) {
            try {
                const records = await CurrentConditions.find({
                    interval: intervalOf(
                        req.params.interval as IntervalDescriptor
                    ),
                })
                    .sort({
                        time: "asc",
                ...
                return next(
                    new APIError(
                        "Failed to access the current weather conditions from the database!",
                        500,
                        err
                    )
                );
        ...
        if (result.isEmpty()) {
            ...
            try {
                ...
                if (err instanceof mongoose.Error.ValidationError) {
                    ...
                    return next(new APIError(message, 400, req.body));
                } else {
                    return next(
                        new APIError(
                            "Unknown error while validation!",
                            500,
                            err
                        )
                    );
            ...
            try {
                ...
                return next(
                    new APIError("Error while processing record!", 500, err)
                );
            }
